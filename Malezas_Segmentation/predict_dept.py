"""
Este código realiza estimación de profundidad para imágenes de Malezas (Malezas_Segmentation).

1. Busca todas las imágenes en INPUT_DIR.
2. Construye un pipeline de estimación de profundidad usando el modelo 'Intel/dpt-large'.
3. Para cada imagen:
   - Lee la imagen.
   - Calcula el mapa de profundidad normalizado.
   - Aplica un colormap para visualización.
   - Guarda la imagen resultante en OUTPUT_DIR.
4. Devuelve la ubicación de los mapas de profundidad generados.
...
INPUT_DIR  = r"Malezas-detection-2-1/test_images_train"   
...
def infer_depth01(pipe, bgr: np.ndarray, clip_perc: Tuple[int,int]) -> np.ndarray:
    ...
    depth_vis = np.array(out["depth"]).astype(np.float32)   #  0..255 visual
